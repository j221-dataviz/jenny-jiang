...
highchart() %>% 
  hc_title(text = "This is a bar graph describing my favorite pies
           including a pie chart describing my favorite bars") %>%
  hc_subtitle(text = "In percentage of tastiness and awesomeness") %>% 
  hc_add_series_labels_values(year_1011$job, year_1011$sum_salary, name = "Pie",
                              colorByPoint = TRUE, type = "column") %>% 
  hc_add_series_labels_values(year_1011$job, year_1011$sum_salary, type = "pie",
                              name = "Bar", colorByPoint = TRUE, center = c('35%', '10%'),
                              size = 100, dataLabels = list(enabled = FALSE)) %>% 
  hc_yAxis(title = list(text = "percentage of tastiness"),
           labels = list(format = "{value}%"), max = 100) %>% 
...
year_1011 %>%
  mjs_plot(x=sum_salary, y=job) %>%
  mjs_bar() 



ggplot(year_1011, aes(x=job, y=sum_salary) )+
  geom_bar()
...
median_salary <- highchart() %>% 
  ...
  hc_add_series_labels_values(year_1415$job, year_1011$sum_salary, name = "2014-15",
                              colorByPoint = TRUE, type = "column", max = 8000) %>% 
